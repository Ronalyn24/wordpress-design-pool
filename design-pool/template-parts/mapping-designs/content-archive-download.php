...
<div id="main-archive-downloads">

	<aside class="sfp-scroll-top"></aside>

<?php // if download taxonomy archive, output information about the term
if ( is_tax( 'download_category' ) || is_tax( 'download_tag' ) ) {
	...
	<?php if ( ! empty( $term_type ) ) { ?>
		<div class="download-term-header" style="display:none;">
			<h1 class="term-title">
				...
				<div itemscope itemtype="http://schema.org/Product" class="edd_download" id="edd_download_<?php echo get_the_ID(); ?>">
					<div class="edd_download_inner">
						...
						<div class="edd_download_image">
							<?php
								$aw_alt = get_the_title();
								$custom_alt = $aw_alt . " Mapping";
							 	the_post_thumbnail( 'product-image', array( 'alt' => $custom_alt, 'title' => $custom_alt ) ); 
							?>
						</div>
						...
						<?php 
							$normal_edd_user = "<div class='product-buttons'><a class='download-read-more' href='#site-content'>DOWNLOAD</a></div>";
							echo do_shortcode('[edd_user_has_no_purchases loggedout=true]' . $normal_edd_user . '[/edd_user_has_no_purchases]');
						?>

						<div class="custom-edd-button">
							<?php
								$dd_post_id = get_the_ID();
								echo do_shortcode('[edd_user_has_purchased ids="61205,69294"][purchase_link id="' . $dd_post_id . '" price="0" text="Download" style="button" color="blue"][/edd_user_has_purchased]');
							?>
						</div>
